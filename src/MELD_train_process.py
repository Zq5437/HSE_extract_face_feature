...
save_path_prefix = "/home/h666/Zq/MELD_result"
log1_path = "/home/h666/Zq/MELD_result/log1.csv"
log2_path = "/home/h666/Zq/MELD_result/log2.csv"
...
# 获取训练集的文件
train_raw_data_prefix = "/home/h666/Zq/情绪识别数据集/MELD.Raw/train_splits"
train_raw_data_files_name = os.listdir(train_raw_data_prefix)
train_raw_data_files_name = natsorted(train_raw_data_files_name)

# 测试使用，只取前几个文件试试
# train_raw_data_files_name = train_raw_data_files_name[:200]

# 定义一个字典，用于存储训练集的分隔信息
dia = {}

# 分隔训练集文件
for file_name in train_raw_data_files_name:
    # print(file_name)
    dia_begin = 3
...
for dia_number in tqdm(dia, desc="Processing dia:"):
    ...
    for utt_number in tqdm(dia[dia_number], desc=f"Processing dia{dia_number}:", leave=False):
        ...
        video_path = train_raw_data_prefix + "/" + video_name
        
        result, feature = get_numpy_all(train_raw_data_prefix, video_name, log2_path)
        ...
        if result is None:
            ...
            df.to_csv(log1_path, index=False)
            if feature is None:
                _, contrast_feature = get_numpy_all_whole(train_raw_data_prefix, video_name, log2_path)
